...
public class RequestLimitInterceptor implements HandlerInterceptor {
    ...
    @Override
    public synchronized boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws MaximumRequestPerMinuteExceededException {
        ...
        if (method.isAnnotationPresent(Limited.class) ||
            ...
            String params = String.join(",", Lists.newArrayList(
                    method.getParameterTypes()).stream().map(x -> x.getSimpleName()).collect(Collectors.toList()));
