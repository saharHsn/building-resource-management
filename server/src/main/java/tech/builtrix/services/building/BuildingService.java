...
public class BuildingService extends GenericCrudServiceBase<Building, BuildingRepository> {
	...
	public BuildingService(BuildingRepository buildingRepository, UserService userService, FileUploader fileUploader,
			BillParser parser, BillService billService, MessageManager messageManager) {
	...
	private void makeFileUploadEvent(BuildingDto buildingDto) {
		...
		List<String> fileNames = uploadFile(bucketName, buildingId, buildingDto.getElectricityBill(),
	...
	private List<String> uploadFile(String bucketName, String pathName, MultipartFile file, BillType billType) {
		if (file != null) {
			...
			if (file.getContentType() != null && file.getContentType().equalsIgnoreCase("application/zip")) {
				logger.info("file:" + file.getOriginalFilename() + " is zipped!");
				...
				// fileNames.addAll(uploadMultiFiles(destFiles, bucketName, pathName,
				// metaData));
			} else {
				return this.fileUploader.uploadFile(file, bucketName, pathName + "-" + file.getOriginalFilename(),
						metaData);
			}
		}
		return null;
	...
	private List<String> uploadMultiFiles(File destination, String bucketName, String pathName,
		...
		for (File directoryFile : directoryFiles) {
			...
			fileNames.addAll(this.fileUploader.uploadFile(multiPartFile, bucketName,
					pathName + "-" + multiPartFile.getName(), metaData));
		}
		return fileNames;
	}
