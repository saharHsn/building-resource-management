...
public class UserService extends GenericCrudServiceBase<User, UserRepository> {
	...
	@Autowired
	public UserService(UserRepository repository, CodeService codeService, ValidationService validationService,
			EmailSender emailSender, VerificationTokenRepository tokenRepository) {
		super(repository);
	...
	public User registerUserViaInvitation(InvitationDto invitationDto, User parent) throws AlreadyExistException {
		if (emailExists(invitationDto.getInviteeEmail())) {
			throw new AlreadyExistException(
					"There is an account with that email address: " + invitationDto.getInviteeEmail());
		}
		User user = new User();
	...
	private User updateUserByEmail(String firstName, String lastName, String email, String password,
	...
	/*
	 * public User registerNewUserAccount(final UserDto accountDto) { if
	 * (emailExists(accountDto.getEmail())) { throw new
	 * UserAlreadyExistException("There is an account with that email adress: " +
	 * accountDto.getEmail()); } final User user = new User();
	 * 
	 ...
	 * user.setRoles(Arrays.asList(roleRepository.findByName("ROLE_USER"))); return
	...
	/*
	 * public void deleteUser(final User user) { final VerificationToken
	 ...
	 * 
	 * if (passwordToken != null) { passwordTokenRepository.delete(passwordToken); }
	 * 
	...
	/*
	 * public PasswordResetToken getPasswordResetToken(final String token) { return
	 * passwordTokenRepository.findByToken(token); }
	 */

	/*
	 * public User getUserByPasswordResetToken(final String token) { return
	 * passwordTokenRepository.findByToken(token) .getUser(); }
	 */
	...
	public String generateQRUrl(User user) throws UnsupportedEncodingException {
		...
		return QR_PREFIX + URLEncoder.encode(String.format("otpauth://totp/%s:%s?secret=%s&issuer=%s", APP_NAME,
				user.getEmailAddress(), user.getSecret(), APP_NAME), "UTF-8");
	}

	/*
	 * public User updateUser2FA(boolean use2FA) { final Authentication curAuth =
	 * SecurityContextHolder.getContext() .getAuthentication(); User currentUser =
	 * (User) curAuth.getPrincipal(); //currentUser.setUsing2FA(use2FA); currentUser
	 * = repository.save(currentUser); final Authentication auth = new
	 ...
	 * public User updateUser2FA(boolean use2FA) { final Authentication curAuth =
	 * SecurityContextHolder.getContext() .getAuthentication(); User currentUser =
	 * (User) curAuth.getPrincipal(); currentUser.setIsUsing2FA(use2FA); currentUser
	 * = repository.save(currentUser); final Authentication auth = new
	 * UsernamePasswordAuthenticationToken(currentUser, currentUser.getPassword(),
	 * curAuth.getAuthorities()); SecurityContextHolder.getContext()
	 * .setAuthentication(auth); return currentUser; }
