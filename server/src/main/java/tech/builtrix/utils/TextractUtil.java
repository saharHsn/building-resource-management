...
public class TextractUtil {
    public static List<MyTable> extractTables(Map<String, Block> blocksMap, List<Block> tableBlocks) {
        ...
        for (Block tableBlock : tableBlocks) {
            ...
            // csv += generateTableCsv(tableBlock, blocksMap,
            // tableBlocks.indexOf(tableBlock) + 1);
            // csv += "\n\n";
    ...
    private static MyTable generateTable(Block tableBlock, Map<String, Block> blocksMap) {
        ...
        //TODO we have a great problem here!!! for duplicated columnValues
        for (Integer rowIndex : rowsColumnsMap.keySet()) {
            List<String> columnValues = rowsColumnsMap.get(rowIndex);
            String key = columnValues.get(0);
            if (column_value.containsKey(key)) {
                /*List<String> values = column_value.get(key);
                List<String> newList = mergeLists(values, columnValues.subList(1, columnValues.size()));
                column_value.put(key, newList);
                System.out.println();*/
                column_value.put(key + "-row" + rowIndex, columnValues.subList(1, columnValues.size()));
            } else {
                column_value.put(key, columnValues.subList(1, columnValues.size()));
            }
        }
    ...
    public static Map<String, String> extractKeyValues(Map<String, Block> blocksMap, Map<String, Block> keysMap,
                                                       Map<String, Block> valuesMap) {
    ...
    private static Map<String, String> get_kv_relationship(Map<String, Block> key_map, Map<String, Block> value_map,
