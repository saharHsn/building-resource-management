...
public class ApplicationExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    ResponseEntity handleDomainException(ExceptionBase ex) {
        ...
        if (errorMessage == null) {
            logger.warn(String.format("Exception of type [%s] has been thrown without ErrorMessage annotation", ex.getClass().getSimpleName()));
            return new ResponseEntity<Object>(Response.error(500,
                    createUserMessage(ex, ex.getMessage()),
                    createDeveloperMessage(ex, ex.getMessage())), HttpStatus.INTERNAL_SERVER_ERROR);
        } else {
            return new ResponseEntity<Object>(Response.error(errorMessage.code(),
                    createUserMessage(ex, errorMessage.message()),
                    createDeveloperMessage(ex, errorMessage.developerMessage())), errorMessage.status());
        }
    }
    ...
    private String createDeveloperMessage(ExceptionBase ex, String developerMessage) {
        ...
        if (developerMessage.startsWith("${")) {
            developerMessage = localizationService.localizedText(developerMessage.substring(2, developerMessage.length() - 1));
        }
        if (StringUtils.isEmpty(developerMessage)) {
            // eg errors.invalidtoken.dev
            developerMessage = "errors." + ex.getClass().getSimpleName().toLowerCase().replace("exception", "") + ".dev";
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(
        ...
        if (ex.getCause() instanceof InvalidFormatException) {
            ...
            String path = String.join(".", invalidFormatException.getPath().stream().map(x -> x.getFieldName()).collect(Collectors.toList()));
            ...
            if (invalidFormatException.getTargetType().equals(Integer.class) ||
                ...
                response.addError(String.format("Invalid value for [%s] of type number", path, invalidFormatException.getTargetType().getSimpleName()));
            }
            if (invalidFormatException.getTargetType().isArray() ||
                ...
                response.addError(String.format("Invalid value for [%s] of type array", path, invalidFormatException.getTargetType().getSimpleName()));
            }
        }
        if (ex.getCause() == null) {
